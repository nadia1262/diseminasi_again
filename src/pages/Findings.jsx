...
import { Heart, TrendingUp, Lightbulb, CheckCircle, Eye, Sparkles, BookOpen, BarChart3, Layers, MapPin, Users, Home, Droplet, Stethoscope, Paperclip, Image, Camera, Bookmark, StickyNote, Star, Compass } from "lucide-react";
import "./Findings.css";
...
const loadDistrictsData = async () => {
  try {
    const url = new URL('../assets/web-story-2/R3P-Kab-GeoJSON.geojson', import.meta.url).href;
    ...
    const filteredFeatures = geojson.features.filter(
      f => targetProvinces.includes(f.properties.nmprov)
    );
    ...
    console.log(`Districts loading complete: ${allFeatures.length} features loaded from R3P-Kab-GeoJSON`);
...
function Findings() {
  ...
  const needsData = [
    {
      ...
      description: "Akses air bersih adalah prioritas utama untuk kesehatan dan kebersihan harian warga terdampak.",
      stats: "8,450",
      statsLabel: "keluarga membutuhkan",
      percentage: 78,
      detail: "Sumber air tercemar akibat banjir dan longsor di 3 provinsi terdampak."
    },
    {
      id: 2,
      ...
      description: "Rumah yang aman dan layak huni menjadi fondasi pemulihan berkelanjutan untuk masa depan yang lebih baik.",
      stats: "6,200",
    ...
    },
    {
      ...
      description: "Layanan kesehatan yang terjangkau dan berkualitas memastikan pemulihan fisik dan mental masyarakat.",
      stats: "3,580",
      statsLabel: "orang perlu bantuan medis",
      percentage: 52,
      detail: "Puskesmas dan fasilitas kesehatan turut rusak terdampak bencana."
    }
  ];
    ...
    const ctx = gsap.context(() => {
      ...
      if (titleRef.current && heroSubtitleRef.current) {
        ...
        titleTl
          ...
          .to(titleRef.current, { opacity: 1, x: 15, rotation: 1.5, duration: 0.85, ease: 'power2.out' })
          // Overshoot bounce
          .to(titleRef.current, { x: -8, rotation: -0.9, duration: 0.35, ease: 'power1.inOut' })
          // Settle
          .to(titleRef.current, { x: 3, rotation: 0.4, duration: 0.25, ease: 'power1.inOut' })
          .to(titleRef.current, { x: 0, rotation: 0, duration: 0.2, ease: 'power1.out' })
          // Micro-shake
          .to(titleRef.current, { x: 1.5, rotation: 0.4, duration: 0.08, ease: 'none' })
          .to(titleRef.current, { x: -1, rotation: -0.3, duration: 0.08, ease: 'none' })
          .to(titleRef.current, { x: 0.5, rotation: 0.15, duration: 0.06, ease: 'none' })
          .to(titleRef.current, { x: 0, rotation: 0, duration: 0.06, ease: 'none' });
        ...
        subTl
          ...
          .to(heroSubtitleRef.current, { opacity: 1, x: -12, rotation: -1, duration: 0.65, ease: 'power2.out' })
          // Overshoot bounce
          .to(heroSubtitleRef.current, { x: 6, rotation: 0.7, duration: 0.3, ease: 'power1.inOut' })
          // Settle
          .to(heroSubtitleRef.current, { x: -2, rotation: -0.25, duration: 0.22, ease: 'power1.inOut' })
          .to(heroSubtitleRef.current, { x: 0, rotation: 0, duration: 0.18, ease: 'power1.out' })
          // Micro-shake (slightly different pattern for variety)
          .to(heroSubtitleRef.current, { x: -1.2, rotation: -0.3, duration: 0.08, ease: 'none' })
          .to(heroSubtitleRef.current, { x: 0.8, rotation: 0.2, duration: 0.08, ease: 'none' })
          .to(heroSubtitleRef.current, { x: -0.4, rotation: -0.1, duration: 0.06, ease: 'none' })
          .to(heroSubtitleRef.current, { x: 0, rotation: 0, duration: 0.06, ease: 'none' });
      }
      ...
      if (heroPhotoRef.current) {
        gsap.fromTo(heroPhotoRef.current,
          ...
          {
            opacity: 0.3,
            duration: 3, ease: 'power1.inOut', delay: 0.8,
          }
        );
      }
      ...
      scrapbookEls.forEach((el, i) => {
        gsap.fromTo(el,
          { opacity: 0, scale: 0, rotation: () => Math.random() * 360 },
          {
            opacity: 1,
            scale: 1,
            rotation: el.style.transform ? parseFloat(el.style.transform.match(/rotate\((.+)deg\)/)?.[1] || 0) : 0,
            duration: 0.8,
            ease: 'back.out(2)',
            delay: 1.2 + i * 0.1,
          }
      ...
      if (skalaSection.current) {
        ...
        const skalaMapWrap = skalaSection.current.querySelector('.skala-map-container');

        if (skalaMapWrap) {
          gsap.fromTo(skalaMapWrap,
            { filter: 'blur(8px)' },
            {
              filter: 'blur(0px)',
              duration: 1,
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          const step = Math.min(totalSkalaSteps - 1, Math.floor(self.progress * totalSkalaSteps));
      ...
      if (temuanSection.current && temuanTrack.current) {
        ...
        const temuanTitle = temuanSection.current.querySelector('.temuan-title');
        const temuanQuote = temuanSection.current.querySelector('.temuan-quote');
        const temuanChart = temuanSection.current.querySelector('.temuan-chart-container');

        // Title & quote entrance
        if (temuanTitle) {
          gsap.fromTo(temuanTitle,
            { opacity: 0, y: 50 },
            {
      ...
      if (totalSlides > 0) {
        ...
        infogSlides.slice(1).forEach(slide => gsap.set(slide, { xPercent: 110, opacity: 0 }));

        // Animate inner elements of first visible slide
        const firstItems = infogSlides[0].querySelectorAll('.infog-stat, .infog-callout, .infog-slide-title, .infog-story');
        gsap.fromTo(firstItems, { y: 30, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.08, duration: 0.5, ease: 'power2.out', delay: 0.2 });
        ...
        const impactTl = gsap.timeline({
          scrollTrigger: {
            ...
            scrub: 0.6,
            onUpdate: (self) => {
              const idx = Math.min(totalSlides - 1, Math.round(self.progress * (totalSlides - 1)));
        ...
        for (let i = 0; i < totalSlides - 1; i++) {
          ...
          impactTl
            ...
            .fromTo(infogSlides[i + 1],
              { xPercent: 110, opacity: 0, scale: 0.85, rotation: 3 },
              { xPercent: 0, opacity: 1, scale: 1, rotation: 0, duration: 1, ease: 'power3.inOut' },
              '<0.15'
            );
          impactTl.to({}, { duration: 0.35 });
        }
      ...
      evacPhotos.forEach((frame, i) => {
        ...
        tl.fromTo(frame,
          { opacity: 0, x: startX, y: 100, rotation: startRot, scale: 0.4, filter: 'sepia(1) brightness(1.5) blur(5px)' },
          { opacity: 0.3, x: startX * 0.6, y: 65, rotation: startRot * 0.6, scale: 0.55, filter: 'sepia(0.8) brightness(1.3) blur(3px)', duration: 0.1, ease: 'steps(1)' }
        )
        .to(frame, { opacity: 0.55, x: startX * 0.25, y: 35, rotation: startRot * 0.2, scale: 0.75, filter: 'sepia(0.4) brightness(1.1) blur(1.5px)', duration: 0.1, ease: 'steps(1)' })
        .to(frame, { opacity: 0.3, y: 30, scale: 0.77, filter: 'sepia(0.3) brightness(0.85) blur(1px)', duration: 0.05, ease: 'steps(1)' })
        .to(frame, { opacity: 0.8, x: 0, y: 8, rotation: 0, scale: 0.95, filter: 'sepia(0.1) brightness(1.05) blur(0px)', duration: 0.12, ease: 'steps(1)' })
        .to(frame, { opacity: 1, y: -6, scale: 1.03, filter: 'sepia(0) brightness(1) blur(0px)', duration: 0.1, ease: 'steps(1)' })
        .to(frame, { y: 0, scale: 1, rotation: (Math.random() - 0.5) * 3, duration: 0.08, ease: 'steps(1)' });

        // Idle jitter
        const jitter = gsap.timeline({ repeat: -1, yoyo: true, delay: 1 + i * 0.5 });
        jitter
          .to(frame, { y: -2, rotation: `+=${(Math.random() - 0.5) * 2.5}`, duration: 2.2, ease: 'steps(3)' })
          .to(frame, { y: 1.5, rotation: `+=${(Math.random() - 0.5) * 2}`, duration: 1.8, ease: 'steps(2)' });
      });
      ...
      evacStatCards.forEach((card, i) => {
        gsap.fromTo(card,
          { opacity: 0, y: 40, scale: 0.85 },
          {
            ...
            scrollTrigger: { trigger: card, start: 'top 88%', end: 'top 60%', scrub: 0.5 },
            delay: i * 0.08,
          }
        );
      });
        ...
        if (underline) {
          gsap.fromTo(underline,
            { scaleX: 0, transformOrigin: 'center center' },
            {
              ...
              scrollTrigger: { trigger: sec, start: 'top 65%', end: 'top 35%', scrub: 0.5 }
            }
          );
        }
        ...
        cards.forEach((card, i) => {
          ...
          if (statNumber) {
            ...
            cardTl.to(obj, {
              ...
              ease: 'power2.out',
              onUpdate: () => {
                statNumber.textContent = Math.round(obj.val).toLocaleString('id-ID');
              }
            }, '-=1.2');
          }
        ...
        if (closingText) {
          gsap.fromTo(closingText,
            { opacity: 0, y: 50, scale: 0.95 },
            {
              ...
              scrollTrigger: { trigger: closingText, start: 'top 90%', toggleActions: 'play none none reverse' }
            }
          );
        }
      ...
      const insightToInterview = document.querySelector('.insight-to-interview');
      if (insightToInterview) {
        gsap.fromTo(insightToInterview.querySelector('svg path'),
      ...
      if (interviewTitle) {
        gsap.fromTo(interviewTitle,
          { opacity: 0, y: 40 },
          { opacity: 1, y: 0, duration: 1, ease: 'power2.out',
            scrollTrigger: { trigger: '.interview-section', start: 'top 80%', end: 'top 50%', scrub: 0.6 }
          }
        );
      }
      if (interviewSubtitle) {
        gsap.fromTo(interviewSubtitle,
          { opacity: 0, y: 30 },
          { opacity: 1, y: 0, duration: 1, ease: 'power2.out',
            scrollTrigger: { trigger: '.interview-section', start: 'top 75%', end: 'top 45%', scrub: 0.6 }
          }
        );
      }
      ...
      const closingImage = document.querySelector('.interview-closing-img-wrap');
      const closingQuote = document.querySelector('.interview-closing-quote');
      const closingQuoteText = document.querySelector('.interview-closing-quote-text');

      if (closingImage) {
        gsap.fromTo(closingImage,
          { opacity: 0, y: 60 },
          {
            opacity: 1, y: 0, duration: 1, ease: 'power2.out',
            scrollTrigger: {
      ...
      if (closingQuote) {
        ...
        quoteTl
          ...
          .to(closingQuote, { opacity: 0.3, y: 55, x: -25, rotation: 1.2, duration: 0.25, ease: 'power2.out' })
          .to(closingQuote, { opacity: 0.7, y: 25, x: -10, rotation: 0.4, duration: 0.25, ease: 'power2.out' })
          .to(closingQuote, { opacity: 1, y: -10, x: 5, rotation: -0.2, duration: 0.3, ease: 'power2.out' })
          .to(closingQuote, { y: 3, x: 0, rotation: 0.1, duration: 0.2, ease: 'power1.inOut' })
          .to(closingQuote, { y: 0, x: 0, rotation: 0, duration: 0.15, ease: 'power1.out' });
      }

      if (closingQuoteText) {
        gsap.fromTo(closingQuoteText,
          { clipPath: 'inset(0 100% 0 0)' },
          {
            clipPath: 'inset(0 0% 0 0)',
            duration: 0.8, ease: 'steps(25)',
            scrollTrigger: {
              trigger: '.interview-closing-wrap',
              start: 'top 65%',
              end: 'top 40%',
      ...
      if (closingSec) {
        ...
        gsap.fromTo(closingSec.querySelector('.closing-collage'),
          { scale: 1.25 },
          {
            scale: 1,
            ease: 'none',
        ...
        gsap.fromTo(closingSec.querySelector('.closing-bg-overlay'),
          { opacity: 0.3 },
          {
            opacity: 0.85,
            ease: 'none',
        ...
        if (closingTag) {
          gsap.fromTo(closingTag,
            { opacity: 0, y: 30 },
            { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out',
              scrollTrigger: { trigger: closingContent, start: 'top 75%', toggleActions: 'play none none reverse' }
            }
          );
        }
        ...
        if (closingTitle) {
          gsap.fromTo(closingTitle,
            ...
            { opacity: 1, y: 0, scale: 1, duration: 1, ease: 'power3.out',
              scrollTrigger: { trigger: closingContent, start: 'top 70%', toggleActions: 'play none none reverse' }
            }
          );
        }

        if (closingQuoteEl) {
          gsap.fromTo(closingQuoteEl,
            { opacity: 0, y: 30 },
            { opacity: 1, y: 0, duration: 1, delay: 0.2, ease: 'power2.out',
              scrollTrigger: { trigger: closingContent, start: 'top 65%', toggleActions: 'play none none reverse' }
            }
          );
        }

        if (closingDivider) {
          gsap.fromTo(closingDivider,
            { scaleX: 0 },
            { scaleX: 1, duration: 1.2, ease: 'power2.inOut',
              scrollTrigger: { trigger: closingContent, start: 'top 60%', toggleActions: 'play none none reverse' }
            }
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const skalaMapEl = skalaSection.current.querySelector('.skala-map-container');
    ...
    const timer = setTimeout(() => {
      try {
        ...
        skalaMap.on('load', () => {
          ...
          const targetProvinces = ['ACEH', 'SUMATERA UTARA', 'SUMATERA BARAT'];
          const filteredFeatures = r3pKabData.features.filter(
            f => targetProvinces.includes(f.properties.nmprov)
          );
          ...
          skalaMap.addLayer({
            ...
            paint: {
              'fill-color': [
                ...
                ['get', 'nmprov'],
                'ACEH', '#FF6B6B',
                'SUMATERA UTARA', '#FFA500',
                'SUMATERA BARAT', '#FFD166',
                '#FFEA8A'
              ],
              'fill-opacity': 0
            }
          ...
          regionsData.forEach((region, idx) => {
            ...
            const marker = new maplibregl.Marker({ element: markerDiv, anchor: 'center' })
          ...
          setTimeout(() => {
            ...
            const animateBoundaries = (timestamp) => {
              ...
              try {
                skalaMap.setPaintProperty('skala-stroke', 'line-opacity', eased * 0.9);
                skalaMap.setPaintProperty('skala-stroke', 'line-width', 1 + eased * 2.5);
                skalaMap.setPaintProperty('skala-fill', 'fill-opacity', eased * 0.25);
              } catch(e) {}
  ...
  useEffect(() => {
    ...
    if (skalaStep === 0) {
      mapInstance.flyTo({ center: [98.5, 2.0], zoom: 7, duration: 1500, essential: true });
      ...
      markers.forEach(m => { m.style.opacity = '0'; m.style.pointerEvents = 'none'; });

      // Hide district layers
      try {
        mapInstance.setPaintProperty('skala-districts-fill', 'fill-opacity', 0);
        mapInstance.setPaintProperty('skala-districts-stroke', 'line-opacity', 0);
      } catch(e) {}
      ...
      if (skalaContent) gsap.to(skalaContent, { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' });
    ...
    if (skalaStep === 1) {
      ...
      const pmlOverlay = document.querySelector('.skala-pml-overlay');
      const overlayUI = document.querySelector('.skala-overlay-ui');
      if (skalaContent) gsap.to(skalaContent, { opacity: 0, y: -30, duration: 0.4 });
      if (pmlOverlay) gsap.to(pmlOverlay, { opacity: 1, y: 0, duration: 0.5, delay: 0.2, ease: 'power2.out' });
      if (overlayUI) gsap.to(overlayUI, { opacity: 0, duration: 0.3 });
    ...
    if (skalaStep >= 2 && region) {
      ...
      const pmlOverlay = document.querySelector('.skala-pml-overlay');
      const overlayUI = document.querySelector('.skala-overlay-ui');
      if (skalaContent) gsap.to(skalaContent, { opacity: 0, y: -30, duration: 0.3 });
      if (pmlOverlay) gsap.to(pmlOverlay, { opacity: 0, y: 20, duration: 0.3 });
      if (overlayUI) gsap.to(overlayUI, { opacity: 1, duration: 0.4, delay: 0.2 });
      ...
      // Animate stats cards
      const allStatCards = document.querySelectorAll('.skala-section .map-stats-card');
      allStatCards.forEach((card, i) => {
        if (i === activeRegion) {
          gsap.fromTo(card,
            { y: 80, opacity: 0, visibility: 'hidden' },
            { y: 0, opacity: 1, visibility: 'visible', duration: 0.6, ease: 'power3.out', delay: 0.3 }
          );
        } else {
          gsap.to(card, {
      ...
      // Animate photo cards
      const allPhotoCards = document.querySelectorAll('.skala-section .map-photo-card');
      allPhotoCards.forEach((card, i) => {
        if (i === activeRegion) {
          gsap.fromTo(card,
            { y: 100, opacity: 0, visibility: 'hidden' },
            { y: 0, opacity: 1, visibility: 'visible', duration: 0.7, ease: 'power3.out', delay: 0.4 }
          );
        } else {
          gsap.to(card, {
      ...
      if (districtsData) {
        ...
        try {
          ...
          if (source && regionDistricts.length > 0) {
            source.setData({ type: 'FeatureCollection', features: regionDistricts });
            mapInstance.setPaintProperty('skala-districts-fill', 'fill-opacity', 0.25);
            mapInstance.setPaintProperty('skala-districts-stroke', 'line-opacity', 0.7);
          }
        } catch(e) {}
      }
    }
  ...
  return (
    <div ref={mainContainer} className="findings-container">
      ...
      <section ref={heroSection} className="hero-section hero-section--map">
        ...
        <div ref={heroPhotoRef} className="hero-photo-overlay" style={{ backgroundImage: `url(${bencana1})` }}></div>
        ...
        <div className="hero-content hero-content--map">
          ...
          <p ref={heroSubtitleRef} className="hero-subtitle hero-subtitle--story">
            Bencana sering tampak sebagai titik dan garis di peta. Tapi ruang menyimpan lebih dari itu — ia menyimpan cara hidup, ingatan, dan hubungan yang tiba-tiba bergeser. Cerita ini menelusuri dampak dari ruang, data, dan manusia.
          </p>
      ...
      <section ref={skalaSection} className="skala-section">
        ...
        <div className="skala-content">
          ...
          <p className="skala-text">
            Kerusakan tidak berhenti di satu titik. Ia menjalar mengikuti sungai, jalan, dan pola permukiman — seperti riak yang terus meluas setelah batu pertama jatuh.
          </p>
          <div className="skala-legend">
            <div className="skala-legend-item"><span className="skala-dot" style={{background:'#FF6B6B'}}></span>Aceh</div>
            <div className="skala-legend-item"><span className="skala-dot" style={{background:'#FFA500'}}></span>Sumatera Utara</div>
            <div className="skala-legend-item"><span className="skala-dot" style={{background:'#FFD166'}}></span>Sumatera Barat</div>
          </div>
        </div>
            ...
            {regionsData.map((region, index) => (
              ...
              >
                <div className="msc-header">
                  <div className="msc-icon"><region.icon size={20} /></div>
                  <div>
                    <h3 className="msc-region-name">{region.name}</h3>
                    <span className="msc-cities">{region.cities.join(', ')}</span>
                  </div>
                  <span className="msc-step">{index + 1}/{regionsData.length}</span>
                </div>
                <div className="msc-body">
                  ...
                  <div className="msc-progress">
                    <div className="msc-progress-label">
                      <span>Pemulihan</span><span>{region.progress}%</span>
                    </div>
                    <div className="msc-progress-track">
                      <div className="msc-progress-fill" style={{ width: `${region.progress}%` }}></div>
                    </div>
                  </div>
                  <div className="msc-needs">
                    {region.needs.map(n => <span key={n} className="msc-need-tag">{n}</span>)}
                  </div>
                </div>
              </div>
            ))}
            ...
            {regionsData.map((region, index) => (
              ...
              >
                <div className="mpc-img-wrap">
                  <img src={region.photo} alt={region.name} className="mpc-img" />
        ...
        <div className="skala-step-indicator">
          {['Skala', 'PML/PCL', 'Aceh', 'Sumut', 'Sumbar'].map((label, i) => (
            <div key={i} className={`skala-step-dot ${skalaStep === i ? 'active' : ''}`}>
      ...
      <section ref={temuanSection} className="temuan-section">
        <div className="temuan-header">
          ...
          <h2 className="temuan-title">TEMUAN FISIK — <em>"Apa yang Tersisa"</em></h2>
          <p className="temuan-quote">
            "Retakan pada dinding terlihat pertama, tapi perubahan rasa aman bertahan paling lama."
          </p>
        </div>
        ...
        <div ref={temuanTrack} className="temuan-track">
          ...
          <div className="temuan-photo-card">
            ...
            <p className="temuan-photo-caption">Dinding retak akibat guncangan gempa</p>
          </div>
          <div className="temuan-photo-card">
            <div className="temuan-tape"></div>
            <img src={bencana2} alt="Kerusakan struktur" />
            <p className="temuan-photo-caption">Atap rumah ambruk — struktur tidak bisa diperbaiki</p>
          </div>
          <div className="temuan-photo-card">
            <div className="temuan-tape"></div>
            <img src={bencana3} alt="Dampak longsor" />
            <p className="temuan-photo-caption">Longsor menghancurkan permukiman di lereng</p>
          </div>
          <div className="temuan-photo-card">
            <div className="temuan-tape"></div>
            <img src={bencana4} alt="Bencana banjir" />
            <p className="temuan-photo-caption">Banjir merendam hunian hingga lantai dua</p>
          </div>
          <div className="temuan-photo-card">
            <div className="temuan-tape"></div>
            <img src={warga4} alt="Kondisi warga" />
            <p className="temuan-photo-caption">Warga menyelamatkan barang dari reruntuhan</p>
          </div>
          <div className="temuan-photo-card">
            <div className="temuan-tape"></div>
            <img src={warga5} alt="Dampak pada warga" />
            <p className="temuan-photo-caption">Sisa hunian yang tidak lagi bisa ditempati</p>
          </div>
        ...
        <div className="infog-wrapper">
          ...
          <div className="infog-viewport">
            {disasterData.map((disaster, index) => (
              <div key={disaster.id} className={`infog-slide infog-slide-${index}`}>
                <div className="infog-slide-inner">
                  <div className="infog-visual">
                    <div className="infog-img-frame">
                      <img src={disaster.image} alt={disaster.title} className="infog-img" />
                      <div className="infog-img-overlay"></div>
                    </div>
                    <span className="infog-type-badge">{disaster.description}</span>
                  </div>

                  <div className="infog-content">
                    <div className="infog-slide-num">{String(index + 1).padStart(2, '0')}</div>
                    ...
                    <div className="infog-stats">
                      {disaster.stats.map(stat => (
                        <div key={stat.label} className="infog-stat" style={{ '--accent': stat.color }}>
                          <div className="infog-stat-bar" style={{ background: stat.color }}></div>
                          <span className="infog-stat-value">{stat.value}</span>
                          <span className="infog-stat-suffix">{stat.suffix}</span>
          ...
          <div className="infog-nav">
            {disasterData.map((d, i) => (
              <div key={i} className={`infog-dot ${activeDisaster === i ? 'active' : ''}`}>
      ...
      <section ref={evacuationSection} className="evacuation-section">
        ...
        <div className="evac-hero-parallax">
          <img src={pengungsian} alt="Tenda pengungsian" className="evac-hero-img" />
        ...
        <div className="evac-container">
          <div className="section-header">
            ...
            <h2 className="section-title-text">EVAKUASI — &ldquo;Ruang Sementara&rdquo;</h2>
            <p className="section-desc">&ldquo;Di bawah tenda, hidup berjalan pelan. Ruang mengecil, solidaritas membesar.&rdquo;</p>
          </div>

          {/* Photo gallery — stop motion scrapbook */}
          <div className="evac-photos">
            {evacuationData.map((evac, index) => (
              <div key={`photo-${evac.id}`} className={`evac-photo-frame evac-photo-${index + 1}`}>
                <img src={evac.image} alt={evac.title} className="evac-photo-img" />
          ...
          <div className="evac-stats-grid">
            {evacuationData.map((evac) =>
              evac.stats.map((stat) => {
                ...
                return (
                  <div key={stat.label} className="evac-stat-card" style={{ '--accent': stat.color }}>
                    <div className="evac-stat-card-icon"><IconComp size={22} /></div>
                    <span className="evac-stat-card-val">{stat.value} <small>{stat.suffix}</small></span>
      ...
      <section ref={insightSection} className="insight-section">
        ...
        <div className="insight-particles">
          {[...Array(12)].map((_, i) => (
            <span key={i} className={`insight-particle insight-particle-${i + 1}`} />
          ))}
        </div>
        ...
        <div className="insight-container">
          ...
          <div className="insight-header">
            ...
            <p className="insight-subtitle">
              &ldquo;Tiga kebutuhan yang terus kembali sebagai prioritas utama warga.&rdquo;
            </p>
          </div>
            ...
            {needsData.map((need, index) => {
              ...
              return (
                <div
                  ...
                  style={{ '--card-color': need.color, '--card-color-light': need.colorLight }}
                >
                  {/* Glow ring behind icon */}
                  <div className="insight-glow-ring" />

                  {/* Number badge */}
                  <span className="insight-card-num">{String(index + 1).padStart(2, '0')}</span>
                  ...
                  <div className="insight-card-text">
                    ...
                    <p className="insight-card-description">{need.description}</p>

                    {/* Stat highlight */}
                    <div className="insight-stat-highlight">
                      <span className="insight-stat-number">{need.stats}</span>
                      <span className="insight-stat-label">{need.statsLabel}</span>
                    </div>
                    ...
                    <div className="insight-progress-wrap">
                      <div className="insight-progress-header">
                        ...
                        <span className="insight-progress-pct">{need.percentage}%</span>
                      </div>
          ...
          <div className="insight-closing-text">
            <Lightbulb size={24} />
            <p>Melalui pendataan langsung, kami mengidentifikasi kebutuhan spesifik setiap komunitas untuk memastikan bantuan yang tepat sasaran dan berkelanjutan.</p>
          </div>
        </div>
      </section>
      ...
      <div className="section-transition insight-to-interview">
        <svg viewBox="0 0 1440 120" preserveAspectRatio="none">
          <path d="M0,40 C480,120 960,0 1440,80 L1440,120 L0,120 Z" fill="#0a1a18" />
        </svg>
      </div>
        ...
        <div className="interview-container">
          <div className="interview-header-area">
            <div className="section-header">
              ...
              <h2 className="section-title-text interview-title">Di Balik Angka Ada Suara</h2>
              <p className="section-desc interview-subtitle">&ldquo;Data memberi pola; suara memberi makna.&rdquo;</p>
            </div>
          </div>
          ...
          <div className="interview-scroll-wrap">
            <div className="interview-stack">
              {interviewSlides.map((slide, i) => (
                <div key={i} className={`interview-block interview-block-${i + 1}`}>
                  <div className="interview-block-img-wrap">
                    <img src={slide.image} alt={slide.caption} className="interview-block-img" />
          ...
          <div className="interview-closing-wrap">
            <div className="interview-closing-img-wrap">
              <img src={wawancara} alt="Closing interview" className="interview-closing-img" />
      ...
      <section className="closing-section">
        ...
        <div className="closing-collage">
          <img src={warga1} alt="Warga terdampak" className="closing-collage-img closing-img-1" />
          <img src={warga2} alt="Tim lapangan" className="closing-collage-img closing-img-2" />
          <img src={relawan} alt="Relawan" className="closing-collage-img closing-img-3" />
          <img src={warga3} alt="Warga" className="closing-collage-img closing-img-4" />
          <img src={warga4} alt="Pendataan" className="closing-collage-img closing-img-5" />
          <img src={warga5} alt="Solidaritas" className="closing-collage-img closing-img-6" />
        </div>
        <div className="closing-content">
          <span className="closing-tag">Penutup</span>
          <h2 className="closing-title">Lebih dari Peta</h2>
          <p className="closing-quote">
            &ldquo;Ini bukan hanya arsip kerusakan. Ini catatan tentang ketahanan, solidaritas, dan kemampuan manusia untuk memulai lagi.&rdquo;
          </p>
